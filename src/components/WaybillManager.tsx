...
export const WaybillManager: React.FC<WaybillManagerProps> = ({ waybills, currentUser, onSave, onDelete }) => {
  ...
  const processWithAI = async () => {
    ...
    try {
      ...
      const mimeType = (image.split(';')[0].split(':')[1] || 'image/jpeg') as string;

      const prompt = `این تصویر یک بارنامه یا رسید کالا است.
تمام متن‌ها را بخوان — دست‌نویس، فارسی، انگلیسی، ناخوانا — بهترین تخمین را بزن.
فقط یک JSON خالص برگردان، بدون هیچ توضیح یا backtick:
{"docNumber":"","sender":"","receiver":"","date":"","items":[{"description":"شرح دقیق کالا","quantity":1,"unit":"عدد"}]}
items باید شامل تمام ردیف‌های جدول کالا باشد.`;

      // Gemini API — key از localStorage یا سرور
          ...
          body: JSON.stringify({
            contents: [{
              parts: [
                ...
                { text: `این تصویر یک بارنامه یا رسید کالا است.
تمام متن‌ها را بخوان — دست‌نویس، فارسی، انگلیسی، ناخوانا — بهترین تخمین را بزن.
فقط یک JSON خالص برگردان بدون هیچ توضیح یا backtick:
{"docNumber":"","sender":"","receiver":"","date":"","items":[{"description":"شرح دقیق کالا","quantity":1,"unit":"عدد"}]}
items باید شامل تمام ردیف‌های جدول کالا باشد.` }
      ...
      if (geminiData.error) throw new Error(geminiData.error.message || 'خطای Gemini');
      const rawText = geminiData.candidates?.[0]?.content?.parts?.[0]?.text || '';
      const clean = rawText.replace(/[`]{3}json|[`]{3}/g, '').trim();
      const jsonMatch = clean.match(/\{[\s\S]*\}/);
      if (!jsonMatch) throw new Error('پاسخ JSON نبود: ' + rawText.slice(0,100));
      const result = JSON.parse(jsonMatch[0]);

      if (!result) throw new Error('نتیجه‌ای دریافت نشد');

      if (Array.isArray(result.items) && result.items.length > 0) {
        setExtractedItems(result.items.map((it: any) => ({
          description: String(it.description || ''),
          quantity:    Number(it.quantity) || 1,
          unit:        String(it.unit || 'عدد'),
        })));
      }
      if (result.docNumber) setDocNumber(String(result.docNumber));
      if (result.sender)    setSender(String(result.sender));
      if (result.receiver)  setReceiver(String(result.receiver));
      if (result.date)      setDate(String(result.date));
